...
#define _GGPONET_H_
 ...
 // NOTE: These are a cutpaset from lib/types.h
...
#define GGPO_INVALID_HANDLE      (-1)
   ...
   */
  GGPO_API GGPOSession* __cdecl ggpo_start_session(
   ...
   *
   * totalSize - The size of the controller inputs.  This must be exactly equal to the
   ...
   *
   * The time to wait before the first GGPO_EVENTCODE_CONNECTION_INTERRUPTED timeout
   * will be sent.
   *
   * timeout - The amount of time which needs to elapse without receiving a packet
   *           before the GGPO_EVENTCODE_CONNECTION_INTERRUPTED event is sent.
   */

...
// from replay.cpp
INT32 SaveInputState(UINT8** buf, int* size);
...
INT32 BurnStateLoad(TCHAR* filename, INT32 bAll, INT32(*pLoadGame)())
{
  ...
  if (nReplayStatus)
  {
    ...
    do
    {
      ...
      // LEGACY: dynhuff.cpp data was here, but we don't use that anymore!
      ...
      fread(buf, 1, nChunkSize, fp);
      if (LoadInputState(buf, nChunkSize))
      {
        nRet = -1;
        break;
      }
...
INT32 BurnStateSave(TCHAR* szName, INT32 bAll)
{
  ...
  */
  ...
  if (nReplayStatus != EReplayStatus::REPLAY_STATUS_NONE)
  {
    ...
    // INT32 ret = -1;
    //// NOTE: I think that this can go away.......
    //if (nReplayStatus == EReplayStatus::REPLAY_STATUS_RECORD)
    //{
    ...
    // freeze encode/decode was ok, and so was freeze input.
    if (!SaveInputState(&input_buf, &input_size))
    {
      const char szMovieExtra[] = "MOV ";
      // const char szDecodeChunk[] = "HUFF";
      const char szInputChunk[] = "INP ";
